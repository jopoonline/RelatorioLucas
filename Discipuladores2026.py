...
if 'membros_cadastrados' not in st.session_state:
    st.session_state.membros_cadastrados = {
        "Mirella/Jhow": {n: "Membro" for n in ["Jonathan", "Myrella", "Marco", "Kauê", "Lícia", "Manu", "Leonara", "Alisson", "Giovanna", "Wic", "Waldemir"]},
        "Gustavo": {n: "Membro" for n in ["Gustavo", "Carol", "Ana", "Kelvin", "Cleiton", "Luquinhas"]},
        "Marçal": {n: "Membro" for n in ["Marçal", "Matias", "João Henrique", "Victor", "Bahia", "Guilherme", "Geovane"]}
    }
    st.session_state.membros_cadastrados["Marçal"]["Geovane"] = "FA"
...
with tab_dash:
    if st.session_state.db.empty:
        ...
        with col_l:
            st.write("### 🚨 Radar de Faltas (2 semanas)")
            d_u = sorted(df['Data'].unique())
            if len(d_u) >= 2:
                df_r = df[df['Data'].isin(d_u[-2:])]
                fals = df_r.groupby(['Nome', 'Líder'])['Célula'].sum().reset_index()
                list_v = fals[fals['Célula'] == 0]
                for _, r in list_v.iterrows():
                    st.markdown(f'<div class="radar-card"><b>{r["Nome"]}</b> ({r["Líder"]})<br>2 semanas sem Célula!</div>', unsafe_allow_html=True)
        with col_r:
            st.write("### 🗓️ Consolidado Mensal")
            df_m = df.copy(); df_m['Mês'] = df_m['Data'].dt.strftime('%m-%B')
            evo_m = df_m.groupby('Mês')[['Célula', 'Culto']].sum().reset_index()
            fig_m = px.bar(evo_m, x='Mês', y=['Célula', 'Culto'], barmode='group', color_discrete_map={"Célula": "#00D4FF", "Culto": "#FF4B4B"})
...
with tab_lanc:
    ...
    membros_do_lider = st.session_state.membros_cadastrados[lider_sel]
...
with tab_ob:
    if not st.session_state.db.empty:
        rep_p_cel = st.session_state.db.groupby(['Data', 'Tipo'])['Célula'].sum().unstack(fill_value=0)
        rep_p_cul = st.session_state.db.groupby(['Data', 'Tipo'])['Culto'].sum().unstack(fill_value=0)
        st.write("### Presenças Totais (Célula e Culto)")
